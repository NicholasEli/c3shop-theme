...
<?php if ($view_mode == 'teaser') {
		$ride_type = '';
		if(isset($content['field_ride_types']['#items'])){
			$ride_type = str_replace(" ", "-", $content['field_ride_types']['#items'][0]['value']);
		}else{
			$ride_type = 'powder';
		}
		$product_id = array();
		...
		foreach ($products as $product) {
			...
			//size
			$size_tid = taxonomy_term_load($product->field_commerce_snowboard_size['und'][0]['tid']);
	...
	<a 
		data-ride="<?php print strtolower($ride_type) ; ?> " 
		data-size="<?php print $size ; ?>" 
		data-price="<?php print $price ; ?>" 
		data-stock="<?php print $stock_status ; ?>" 
		class="product-category__card product-category__card--small product-overview__card <?php print $price . ' any-price ' . $size . ' ' . $stock_status . ' all-stock ' . strtolower($ride_type);?> any-ride-type" 
...
<section id="left-container">
		...
		if($supportingInfo){
			print '<div id="product-profile">';
				...
				for($i = 0; $i < count($info['#items']); $i++){
					...
					if(isset($info[$i]['entity']['field_collection_item'][$value]['field_support_info_text'])){
	...
	<div id="product-specifications">
		...
		<?php print render($content['field_product_dimensions']); ?>
	</div>
